...
-- FIXED: Uses 'Number' column instead of 'G_Num'
-- =============================================
...
BEGIN
    ...
    -- Get guild number (FIXED: Uses 'Number' column)
    DECLARE @GuildNumber INT
    SELECT @GuildNumber = [Number] FROM [Guild] WHERE [G_Name] = @GuildName
    
    -- Add master to guild members (FIXED: Uses 'Number' column)
    INSERT INTO [GuildMember] ([Name], [G_Name], [G_Status])
    VALUES (@MasterName, @GuildName, 0x80) -- 0x80 = Guild Master
...
PRINT '  - WZ_CreateGuild (FIXED: Uses Number column)'
