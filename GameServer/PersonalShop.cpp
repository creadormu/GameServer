...
void CPersonalShop::CGPShopItemListRecv(PMSG_PSHOP_ITEM_LIST_RECV* lpMsg,int aIndex) // OK
{
	...
	this->SendMsgBotPsShop(aIndex,bIndex);
...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Coin1 < ((DWORD)lpTarget->Inventory[lpMsg->slot].Coin1) && lpTarget->IsBot == Type_Store)//PCPoint & m_PCPoint
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Insufficient WCoinC");
		return;
	}
...
#if (BOT_STORE == 1)
	if(gObj[aIndex].Type == OBJECT_USER && gObj[bIndex].Type == OBJECT_BOTS)
	{
		lpObj->Coin1 -= lpTarget->Inventory[lpMsg->slot].m_PShopValue; // PCPoint & m_PCPoint
		GDSetCoinSend(lpObj->Index,-(lpTarget->Inventory[lpMsg->slot].m_PShopValue), 0, 0,"BotStore Buy");
	}
#else
	if(gObj[aIndex].Type == OBJECT_USER)
	{
		lpObj->Coin1 -= lpTarget->Inventory[lpMsg->slot].Coin1; // PCPoint & m_PcPointValue
	}
#endif
...
void CPersonalShop::SendMsgBotPsShop(int aIndex,int bIndex)
{
	LPOBJ lpObj = &gObj[aIndex];

	LPOBJ lpTarget = &gObj[bIndex];

	if (lpTarget->IsBot == Type_Store)
	{
			char sbuf[100]={0};
			if(lpObj->BotDefense == 1 && lpObj->Vip != 1)
			{
				LogAdd(LOG_BLUE,"[Account:%s][User:%s] PShop Item Buy Request Failed : Seller is not VIP (%d)",gObj[aIndex].Account,gObj[aIndex].Name);

				gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"This Personal Shop its only for VIP Users");
				return;
			}
			...
			switch(lpTarget->BotPower)
			{
				case 0:
				{
					gNotice.GCNoticeSend(aIndex,1,0,0,0,0,0,"This Personal Shop use Zen");
				}
				break;
				case 1:
				{
			...
			if(BotStore.bot[bIndex].PCPoints > 0)
			{
				gPcPoint.GDPcPointSubPointSaveSend(lpObj->Index,BotStore.bot[bIndex].PCPoints);
				gPcPoint.GDPcPointPointSend(lpObj->Index);
				gNotice.NewNoticeSend(gObj[aIndex].Index,0,0,0,0,0,gMessage.GetMessage(532),BotStore.bot[bIndex].Name,gObj[aIndex].PCPoint);
				GDSetCoinSend(lpObj->Index, - BotStore.bot[aIndex].PCPoints,0,0,"BotStore 2");
			}
		
			LogAdd(LOG_BLUE,"[Account:%s][User:%s] is Receiving PShop List From [BotStore][%s]",gObj[aIndex].Account, gObj[aIndex].Name,lpObj->Name);
