...
bool CObjectManager::CharacterLevelUp(LPOBJ lpObj,DWORD AddExperience,int MaxLevelUp,int ExperienceType) // OK
{
	...
	if(gMasterSkillTree.CheckMasterLevel(lpObj) == 0)
	{
		...
		while(true)
		{
			...
			gAchievements.LevelUp(lpObj->Index);
...
void CObjectManager::CharacterCalcAttribute(int aIndex) // OK
{
	...
	while(change != 0)
	{
		...
		gMuaVip.MUA_VIP_OPTION(lpObj, 1);

		gTitle.Option(lpObj, 1);
		//--
		#if(GAMESERVER_UPDATE>=701)
	...
	if(lpObj->Class == CLASS_DW)
	{
		...
		lpObj->MagicDamageMax += gMuaVip.M_OPTION_MUA_VIP[lpObj->rMuaVip].MUA_VIP_SAT_THUONG;
		lpObj->CurseDamageMax += gMuaVip.M_OPTION_MUA_VIP[lpObj->rMuaVip].MUA_VIP_SAT_THUONG;
		lpObj->MagicDamageMax += gDanhHieu.M_OPTION_DANH_HIEU[lpObj->rDanhHieu].DANH_HIEU_SAT_THUONG;
		lpObj->CurseDamageMax += gDanhHieu.M_OPTION_DANH_HIEU[lpObj->rDanhHieu].DANH_HIEU_SAT_THUONG;
	}
	...
	gAchievements.CalculatorPower(aIndex);
	//--
	gCustomWing.CalcItemCustomOption(lpObj, 0);

	gMuaVip.MUA_VIP_OPTION(lpObj, 0);

	gTitle.Option(lpObj, 0);
	//-----------------------
	#if(GAMESERVER_UPDATE>=701)
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	lpObj->MarryCharacterInfo.Married = lpMsg->MarriedTest;
	memcpy(lpObj->MarryCharacterInfo.MarriedName,lpMsg->MarriedNameTest,sizeof(lpObj->MarryCharacterInfo.MarriedName));
	#endif
	...
	lpObj->rMuaVip = lpMsg->rMuaVip;

	lpObj->rDanhHieu = lpMsg->rDanhHieu;

	lpObj->Lvtuchan = lpMsg->Lvtuchan;

	lpObj->rcrit = lpMsg->rcrit;

	lpObj->rexc = lpMsg->rexc;

	lpObj->rdmg = lpMsg->rdmg;

	lpObj->rdouble = lpMsg->rdouble;
	...
	lpObj->CreateonBank = lpMsg->CreateonBank;
	lpObj->GuardianBank = lpMsg->GuardianBank;
	lpObj->HarmonyBank = lpMsg->HarmonyBank;
	lpObj->LowStoneBank = lpMsg->LowStoneBank;
	lpObj->HighStoneBank = lpMsg->HighStoneBank;
	lpObj->GemStoneBank = lpMsg->GemStoneBank;
	...
	gAchievements.GDPlayerLoad(lpObj->Index);

	gAchievements.GDPlayerSave(lpObj->Index);



	g_ExWinQuestSystem.GC_MiniInfo(lpObj->Index);////////////////////
//	g_ExWinQuestSystem.GC_MiniInfoEx(lpObj->Index,0,0);////////////////////
//	g_ExWinQuestSystem.LoadQuest(lpObj->Index,0);////////////////////
//	g_ExWinQuestSystem.SaveQuest(lpObj->Index,0);////////////////////
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		if(lpTarget->Type == OBJECT_MONSTER)
		{
			...
			gAchievements.MonsterKill(lpObj->Index, lpTarget->Index);
