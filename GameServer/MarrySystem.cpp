...
void Marry::SendInfo(int aIndex)
{
	...
	if(this->Enabled == true)
	{
		...
		ObjYola.PHeadSubSetB((LPBYTE)&lpInfo, 0xFA,0x10, sizeof(lpInfo));
...
void Marry::Propose(LPOBJ lpObj,char * Wife)
{
	...
	{
		gNotice.NewNoticeSend(lpObj->Index,0,0,0,0,0,"[Propose] %s You position is %d,%d!",lpObj->Name,this->ProposeX,this->ProposeY);
			gNotice.NewNoticeSend(gObj[lpTargetID].Index,0,0,0,0,0,"[Propose] %s You position is %d,%d!",gObj[lpTargetID].Name,this->AcceptX,this->AcceptY);}
...
void Marry::CommandCore(LPOBJ lpObj,BYTE Command)
{
	if(this->Enabled == 1)
	{
		if (gServerInfo.m_CmdMarryAF > 0)
		{
			...
			lpObj->MarryCharacterInfo.aFloodMarryCmd = gServerInfo.m_CmdMarryAF;
		}
		...
		switch(Command)
		{
			...
			case 1:
			{
				gCommandManager.AcceptMarry(lpObj);
			}break;
			case 2:
			{
				gCommandManager.DivorceMarry(lpObj);
			}break;
			case 3:
			{
				gCommandManager.ForceDivorceMarry(lpObj);
			}break;
			case 4:
			{
				gCommandManager.TraceMarry(lpObj);
			}break;
			case 5:
			{
				gCommandManager.StatusMarry(lpObj);
			}break;
		}
	}
}
