...
#if (PARTY_SEARCH==1)
	gPartySearch.Load(gPath.GetFullPath("Custom\\PartySearch.txt"));
#endif

#if (CHANGECLASS==1)
	gChangeClass.Load(gPath.GetFullPath("Custom\\ChangeClass.txt"));
#endif	

	gLuckyWheel.Load(gPath.GetFullPath("Custom\\CustomLuckyWheel.txt"));

	SVShop.Init("..\\Data\\Custom\\SmithItem\\SmithItem.ini","..\\Data\\Custom\\SmithItem\\SmithItem.txt");

	gJewelsBank.Load();

	gMarry.LoadData(gPath.GetFullPath("Custom\\MarrySystem.ini"));

	gMuaVip.Load(gPath.GetFullPath("Custom\\CustomMuaVip.txt"));
	gMuaVip.Init();

	gDanhHieu.Load(gPath.GetFullPath("Custom\\CustomDanhHieu.txt"));
	gDanhHieu.Init();

	gTitle.Load(gPath.GetFullPath("Custom\\TuChan.txt"));
...
void CServerInfo::ReadEventInfo() // OK
{
	...
	gSkyEvent.LoadData(gPath.GetFullPath("Event\\SkyEvent.dat"));
	gSkyEvent.ReadMonsters(gPath.GetFullPath("Event\\SkyMonsterGroup.dat"));

	gSwampEvent.LoadDataConfig(gPath.GetFullPath("Event\\SwampEvent.dat"));
	gSwampEvent.ReadMonsters(gPath.GetFullPath("Event\\SwampMonsterGroup.dat"));
...
{
//	gItemManager.OpenItemScript(gPath.GetFullPath("Item\\Item.txt"));

	ObjBotBuff.Read(gPath.GetFullPath("Custom\\BotSystem\\BotBuffer.txt"));
	ObjBotBuff.MakeBot();

	BotAlchemist.Read(gPath.GetFullPath("Custom\\BotSystem\\BotAlchemist.ini"));
	BotAlchemist.MakeBot();

	BotStore.Read(gPath.GetFullPath("Custom\\BotSystem\\BotStore.ini"));
	BotStore.MakeBot();
...
void CServerInfo::ReadCommandInfo(char* section,char* path) // OK
{
	...
	this->m_CommandPostTypeVip = GetPrivateProfileInt(section,"CommandPostTypeVip",0,path); // POST VIP
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	this->m_CharacterCloseCount = GetPrivateProfileInt(section,"CharacterCloseCountTime",0,path);
	...
	this->m_CmdMarryAF = GetPrivateProfileInt(section,"CommanddMarryAntiFlood",0,path);
}
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	...
	this->m_WCOINC_DANHHIEU= GetPrivateProfileInt(section, "WCOINC_DANHHIEU", 0, path);
	//this->m_WCOINC_TULUYEN = GetPrivateProfileInt(section, "WCOINC_TULUYEN", 0, path);
	//this->m_WCOINC_QUANHAM = GetPrivateProfileInt(section, "WCOINC_QUANHAM", 0, path);
	this->m_WCOINC_MUAVIP = GetPrivateProfileInt(section, "WCOINC_MUAVIP", 0, path);
	//this->DoiGioiTinh_WCoinC = GetPrivateProfileInt(section, "DoiGioiTinh_WCoinC", 0, path);
	//---
	...
	//Lucky Wheel
	this->m_LuckyWheelEnabled = GetPrivateProfileInt(section,"LuckyWheelEnabled",0,path);
	this->m_LuckyWheelWCoinC = GetPrivateProfileInt(section,"LuckyWheelWCoinC",0,path);
	this->m_LuckyWheelWCoinP = GetPrivateProfileInt(section,"LuckyWheelWCoinP",0,path);
	this->m_LuckyWheelGPCoin = GetPrivateProfileInt(section,"LuckyWheelGPCoin",0,path);
	this->m_LuckyWheelTime = GetPrivateProfileInt(section,"LuckyWheelTime",0,path);
	this->m_EnablePVPGMStatus = GetPrivateProfileInt(section,"EnablePVPGMStatus",0,path);
}
